...
- **Fallbacks**: on media error the element advances to the next candidate URL (keeps currentTime); video/audio elements track independent candidate indexes; 3s decode watchdog switches codec → lower qn → MP4 single-file mode (RAF sync disabled, MP4 `durl` also goes through `candidates()`)
