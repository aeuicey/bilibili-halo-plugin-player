...
读者端嵌入页（服务端生成的自包含 HTML + Video.js）
  ...
  └─ 流地址三级分发：浏览器直连 / CF Worker 代理 / 服务器代理（直连与服务器代理默认关闭）
```
...
服务器带宽有限时，观众播放不应消耗服务器带宽。嵌入页 `<head>` 带 `<meta name="referrer" content="no-referrer">`，流地址按设置中启用的渠道生成**有序候选数组**，媒体加载失败时递进下一候选并保持播放进度：

| 渠道 | 候选顺序中的位置 | 开关 | 适用场景 |
|------|----------------|------|---------|
| 浏览器直连 | 最优先（若启用） | **默认关闭**，设置中手动开启 | CDN 放行空 Referer，最大化省流 |
| Worker 代理 | 中间（若已配置） | 填写 Worker 地址即启用 | 直连被拦但 Worker 可用 |
| 服务器代理 | 最后（若启用） | **默认关闭**，设置中手动开启 | 最保守，全部流量经服务器中转 |
...
- 直连/服务器代理在插件设置页手动开启（默认均关闭）；Worker 地址与令牌同样在设置页配置，留空时跳过 Worker 层
- 全部渠道未启用时播放页显示「未启用任何流分发渠道」提示，并上报 `noTier` 遥测
...
- **warm 预热模式**（`&warm=1&block=N`）：回源填充指定块但不回传正文（零客户端带宽），返回 `{total}`；播放器据此在后台持续预热整条流——以 `currentTime/duration` 估算播放位置字节优先预热其后的块，seek 后自动改优先级，直到全部块就绪
