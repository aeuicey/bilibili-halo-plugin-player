...
public class VideoController {
    ...
    public Mono<ResponseEntity<String>> embedPlayer(@RequestParam String bvid, @RequestParam(defaultValue = "") String cid) {
        return Mono.fromCallable(() -> {
            ...
            h.append("function playDASH(vUrl,vUrlBak,aUrl,aUrlBak,codecs,w,h){destroyAudio();var vEl=player.el_.querySelector('video');try{vEl.pause()}catch(e){}vEl.removeAttribute('src');vEl.load();vEl.src=pu(vUrl);vEl.load();aEl=document.createElement('audio');aEl.style.display='none';aEl.crossOrigin='anonymous';document.body.appendChild(aEl);aEl.src=pu(aUrl);aEl.load();aEl.muted=true;aEl.volume=player.volume();usingDash=true;tl('dash','v='+codecs.v+' a='+codecs.a+' '+w+'x'+h);if(vUrlBak){vEl.addEventListener('error',function onVErr(){vEl.removeEventListener('error',onVErr);tl('videoFallback','using backup video URL');vEl.src=pu(vUrlBak);vEl.load();if(!player.paused())vEl.play().catch(function(){})},{once:true})}if(aUrlBak){aEl.addEventListener('error',function onAErr(){aEl.removeEventListener('error',onAErr);tl('audioFallback','using backup audio URL');aEl.src=pu(aUrlBak);aEl.load();if(!player.paused())aEl.play().catch(function(){})},{once:true})}var started=false;function tryPlay(reason){if(started)return;started=true;var p=vEl.play();if(p&&typeof p.catch==='function')p.catch(function(err){if(err&&err.name==='AbortError')return;tl('playErr',err.name+':'+err.message);se('播放失败：'+err.message)});tl('playTrigger',reason)}vEl.addEventListener('canplay',function on1(){vEl.removeEventListener('canplay',on1);tryPlay('canplay')},{once:true});vEl.addEventListener('loadedmetadata',function on2(){vEl.removeEventListener('loadedmetadata',on2);setTimeout(function(){tryPlay('metadata')},100)},{once:true});syncLoop()}");

            // RAF-based audio sync loop: follow video time with ±100ms tolerance, handle play/pause/seek
            h.append("function syncLoop(){if(rafId)cancelAnimationFrame(rafId);var stallFrames=0,lastAudioTime=-1;rafId=requestAnimationFrame(function tick(){rafId=requestAnimationFrame(tick);if(!aEl||!usingDash)return;var v=player.el_.querySelector('video');if(!v)return;var dt=v.currentTime-aEl.currentTime;if(Math.abs(dt)>0.08){if(!aEl.paused)aEl.currentTime=v.currentTime}if(v.paused&&!aEl.paused){aEl.pause()}else if(!v.paused&&aEl.paused){aEl.play().catch(function(){})}aEl.volume=v.muted?0:player.volume();aEl.playbackRate=v.playbackRate;if(!v.paused&&aEl.currentTime===lastAudioTime){stallFrames++;if(stallFrames>180){tl('audioStall','reloading audio');aEl.load();aEl.play().catch(function(){});stallFrames=0}}else{stallFrames=0}lastAudioTime=aEl.currentTime})}");

            // Set up video-level event hooks for audio sync
            h.append("function wireAudioHooks(){var v=player.el_.querySelector('video');v.addEventListener('seeked',function(){if(aEl)aEl.currentTime=v.currentTime});v.addEventListener('ratechange',function(){if(aEl)aEl.playbackRate=v.playbackRate});v.addEventListener('volumechange',function(){if(aEl)aEl.volume=v.muted?0:player.volume()});v.addEventListener('timeupdate',function(){if(!aEl||!usingDash)return;var dt=v.currentTime-aEl.currentTime;if(Math.abs(dt)>0.08&&!aEl.paused)aEl.currentTime=v.currentTime})}");
