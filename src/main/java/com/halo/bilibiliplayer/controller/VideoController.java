...
public class VideoController {
    ...
    public Mono<ResponseEntity<String>> embedPlayer(@RequestParam String bvid, @RequestParam(defaultValue = "") String cid) {
        return Mono.fromCallable(() -> {
            ...
            h.append("var player=null,ps=null,cq=0,aq=[],ad=[],firstPlay=true,directMode=false,modeKnown=false,extDetected=false,extChecked=false,extLogin=false,extLoginChecked=false,aEl=null,rafId=0,usingDash=false,unmuteReq=false,flvPlayer=null,playbackMode='',preDataLow=null,vLoadStart=0,aLoadStart=0,cdnStalls=0,cdnStallWindow=0,lastPauseTime=0,userPaused=false,isSeeking=false,seekTime=0,cdnRetries=0,loadPhaseUntil=0,bootRetries=0,pendingDashReq=null,connStepsHidden=false,connStepIndex=1,connStepText='正在检测插件状态';");
            ...
            h.append("console.log('[Bilibili Player] Extension detection started');var extLogOnce=false;window.addEventListener('message',function(e){if(!e.data||e.data.source!=='bilibili-player-extension')return;if(e.data.type==='installed'&&!extLogOnce){extLogOnce=true;console.log('[Bilibili Player] Extension detected');extDetected=true;extChecked=true;setConnectionStep(2,'检测到插件');updateModeStatus();tl('extOk','installed')}if(e.data.type==='status'){if(!extDetected){extDetected=true;extChecked=true;setConnectionStep(2,'检测到插件')}extLogin=e.data.login===true;extLoginChecked=true;console.log('[Bilibili Player] Extension status: login='+extLogin);updateModeStatus();updateResMenu();if(extLogin&&!extLogOnce){extLogOnce=true;tl('extOk','installed+login')}}if(e.data.type==='dashUrl'){if(pendingDashReq&&e.data.reqId===pendingDashReq){pendingDashReq=null;console.log('[Bilibili Player] dashUrl received, ok='+e.data.ok+' error='+(e.data.error||'none'));if(e.data.ok){setConnectionStep(4,'回传成功');playDASH(e.data.videoUrl,null,e.data.audioUrl,null,e.data.codecs,e.data.width,e.data.height)}else{console.warn('[Bilibili Player] dashUrl failed:',e.data.error);tl('dashExtErr',e.data.error||'');if(e.data.error==='not_logged_in'||e.data.error==='login_expired'){showExtensionLoginPrompt()}else{var fbQn=aq.filter(function(q){return q<80}).sort(function(a,b){return b-a})[0]||64;ps.save();loadQuality(fbQn);ps.apply()}}}}});function pingExt(){window.postMessage({source:'bilibili-player',type:'ping'},'*')}pingExt();setTimeout(pingExt,500);setTimeout(pingExt,1500);setTimeout(function(){if(!extChecked){console.log('[Bilibili Player] Extension NOT detected');extChecked=true;setConnectionStep(2,'未检测到插件，使用默认播放策略');updateModeStatus();tl('extMissed','no ext')}},3000);");
            ...
            h.append("player.ready(function(){tl('ready','');function bootQuality(){var extReady=extChecked&&(!extDetected||extLoginChecked);var listReady=aq.length>0;if((!extReady||!listReady)&&bootRetries<20){bootRetries++;setTimeout(bootQuality,300);return}if(extDetected&&extLogin){var hq=aq.filter(function(q){return q>=80})[0];if(hq){loadQuality(hq)}else{loadQuality(64)}}else{loadQuality(64);if(extDetected&&!extLogin)setTimeout(function(){showExtensionLoginPrompt()},2000)}}setTimeout(bootQuality,100);function showUnmute(){var v=player.el_.querySelector('video');if((aEl||flvPlayer)&&v&&v.muted){var btn=document.getElementById('unmuteBtn');if(btn)btn.classList.add('show')}}setTimeout(showUnmute,3000)});");
