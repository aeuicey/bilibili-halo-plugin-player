...
public class VideoController {
    ...
    public Mono<ResponseEntity<String>> embedPlayer(@RequestParam String bvid, @RequestParam(defaultValue = "") String cid) {
        return Mono.fromCallable(() -> {
            ...
            h.append("function syncLoop(){if(rafId)cancelAnimationFrame(rafId);var lastSeek=0,stalled=0,prevTime=-1;rafId=requestAnimationFrame(function tick(){rafId=requestAnimationFrame(tick);if(!aEl||!usingDash)return;var v=player.el_.querySelector('video');if(!v)return;if(v.paused&&!aEl.paused){aEl.pause()}else if(!v.paused&&aEl.paused){aEl.play().catch(function(){})}aEl.volume=v.muted?0:player.volume();aEl.playbackRate=v.playbackRate;var dt=v.currentTime-aEl.currentTime,now=Date.now();if(Math.abs(dt)>0.5&&now-lastSeek>2000){if(!aEl.paused){aEl.currentTime=v.currentTime;lastSeek=now}}if(!v.paused){if(aEl.currentTime===prevTime){stalled++;if(stalled>180){tl('audioStall','reloading');aEl.load();aEl.play().catch(function(){});stalled=0}}else{stalled=0}prevTime=aEl.currentTime}})}");
